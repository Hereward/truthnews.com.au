...
class Director {
	...
	function __construct($api_key = '', $path = '', $debug = false) {
		$this->debug = $debug;
	...
	********************************************************/
	...
	public function send($method, $tail = '') {
	   trigger_error("SEND: HTTP_HOST = {$_SERVER['HTTP_HOST']}");
		if ($this->dead) {
			return array();
		} else {
			...
			if (empty($return)) {
				...
				$this->post = array();
				trigger_error('CURL URL = '.$this->endpoint . $method);
				...
				if (preg_match("/.*uat\..*/", $_SERVER['HTTP_HOST'], $matches)) {
					
					curl_setopt($ch, CURLOPT_PROXY, 'uatdmzproxy.ni.com.au');
                    curl_setopt($ch, CURLOPT_PROXYPORT, '8080');	
				} elseif ($_SERVER['HTTP_HOST'] == 'www.carsguide.com.au' || 
				          $_SERVER['HTTP_HOST'] == 'draft.carsguide.com.au' ||
				          $_SERVER['HTTP_HOST'] == 'carsguide.com.au' ||
				          $_SERVER['HTTP_HOST'] == 'origin.carsguide.com.au' 
				          ) {
					trigger_error("PREG MATCH: HTTP_HOST = {$_SERVER['HTTP_HOST']}");
					curl_setopt($ch, CURLOPT_PROXY, 'proddmzproxy.ni.com.au');
                    curl_setopt($ch, CURLOPT_PROXYPORT, '8080');
