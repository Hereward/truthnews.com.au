...
class Datagrab_model extends CI_Model {
	...
	function do_import( $datatype, $settings ) {
		...
		while( $item = $datatype->next() ) {
			...
			if( isset($this->settings["limit"]) && is_numeric($this->settings["limit"]) && $this->settings["limit"] > 0 ) {
				if ( $row_num - $this->settings["skip"] > $this->settings["limit"] ) {
			...
			// Get date
			$expiry_date = isset( $this->settings[ "expiry_date" ] ) ? $this->settings[ "expiry_date" ] : '';
			If( $datatype->get_item( $item, $expiry_date ) != "" ) {
				$data[ "expiration_date" ] = $this->_parse_date( $datatype->get_item( $item, $expiry_date ) );
			}
			
			// Get URL title
			if( isset( $this->settings[ "url_title" ] ) ) {
				// Get URL title from data source
				$url_title = $datatype->get_item( $item, $this->settings[ "url_title" ] );
				// Add prefix
			...
			// Handle SEO Lite
			if( isset( $this->settings["ajw_seo_lite_title"] ) ) {
				$data["seo_lite__seo_lite_title"] = $datatype->get_item( $item, $this->settings["ajw_seo_lite_title"] );
				$data["seo_lite__seo_lite_keywords"] = $datatype->get_item( $item, $this->settings["ajw_seo_lite_keywords"] );
				$data["seo_lite__seo_lite_description"] = $datatype->get_item( $item, $this->settings["ajw_seo_lite_description"] );
				$data["cp_call"] = TRUE;
			}
			...
			if( isset( $this->settings["ajw_entry_id"] ) && $this->settings["ajw_entry_id"] != "" ) {
				// Check whether entry_id exists
				$e_id = $datatype->get_item( $item, $this->settings["ajw_entry_id"] );
			...
			foreach( $custom_fields as $field => $field_data ) {
				...
				if( isset( $this->settings[ $field ] ) && $this->settings[ $field ] != "" ) {
					...
					if ( class_exists('Datagrab_'.$field_data[ "type" ]) ) {
						$classname = "Datagrab_".$field_data[ "type" ];
						$ft = new $classname();
						// $update = $entry_id > 0 && isset( $this->settings["update"] ) && $this->settings["update"] == "y";
...
// print_r( $data );
		...
		if( $this->batch_limit_completed ) {
			$this->return_data .= "<p>Completed batch of " . $this->settings["limit"] . " entries (offset by " . $this->settings["skip"] . ")</p>";
		}
