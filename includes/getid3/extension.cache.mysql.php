...
class getID3_cached_mysql extends getID3
{
	...
	function analyze($filename) {
		...
		if (file_exists($filename)) {
			...
			// Lookup file
			$select_query = "SELECT `value` FROM `".mysql_real_escape_string($this->table)."` WHERE (`filename` = '".mysql_real_escape_string($filename)."') AND (`filesize` = '".mysql_real_escape_string($filesize)."') AND (`filetime` = '".mysql_real_escape_string($filetime)."')";

			$this->cursor = mysql_query($select_query, $this->connection);
			$numrows = mysql_num_rows($this->cursor);
	...
	function create_table($drop=false) {
        ...
        $query = "CREATE TABLE IF NOT EXISTS `".mysql_real_escape_string($this->table)."` (
        ...
        //$query = "CREATE TABLE IF NOT EXISTS `getid3_cache` ( `filename` VARCHAR(255) NOT NULL DEFAULT '', `filesize` INT(11) NOT NULL DEFAULT '0', `filetime` INT(11) NOT NULL DEFAULT '0', `analyzetime` INT(11) NOT NULL DEFAULT '0', `value` TEXT NOT NULL, PRIMARY KEY (`filename`,`filesize`,`filetime`))";
